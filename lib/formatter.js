...
function formatHaxe(model) {
	...
	str = "package " + model.namespaces[0] + ";\n\n" + imports.join("\n") + "\n\n";
	str += "@:native(\"lib.properties\")\n"
	...
	for(var i in model.classes) {
		...
		str += "@:native(\"" + classDef.name + "\")\n";
		str += "extern class " + cname + " extends " + bname + " {\n";

		if (classDef.bounds) {
			str += "\tstatic public var nominalBounds:Rectangle;\n";
		...
		for(var j in classDef.children) {
			...
			var ctype = safeType(child.type);
			importType(ctype);
			str += "\tpublic var " + child.name + ":" + ctype + ";\n";
		}
		str += "}";
	...
	return {namespace:model.namespaces[0], classes:classes};
}
